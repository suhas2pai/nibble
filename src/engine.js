...
function save() {
  ...
  all[ACCOUNT.username] = { ...ACCOUNT, progress: P };
  store.set(ACCOUNTS_KEY, all);
  store.set(ACCOUNT_KEY, ACCOUNT.username);
}
function usernameKey(username) { return username.trim().toLowerCase().replace(/[^a-z0-9._-]/g, ''); }
function accountProgress(progress) { return Object.assign(fresh(), progress || {}); }
function settings() { return P.settings || (P.settings = { largeText: false, reducedMotion: false, relaxed: false }); }
function applySettings() {
  const s = settings();
  document.documentElement.classList.toggle('large-text', !!s.largeText);
  document.documentElement.classList.toggle('relaxed-mode', !!s.relaxed);
  document.documentElement.dataset.theme = s.reducedMotion ? 'reduced' : '';
}
...
function loseHeart() {
  if (P.settings && P.settings.relaxed) return;
  if (P.pro) return;
...
function dueReview() {
  const today = dayKey();
  return Object.entries(P.review || {}).filter(([, date]) => date <= today).map(([id]) => id);
}
...
function openLesson(id) {
  regen();
  if (!P.pro && !(P.settings && P.settings.relaxed) && P.hearts <= 0) {
...
function topHTML() {
  ...
  <div class="stats">
    ...
    <button class="pill account-pill" data-act="account" aria-label="${ACCOUNT ? 'Account: ' + esc(ACCOUNT.username) : 'Sign in or create an account'}">${ACCOUNT ? '@' + esc(ACCOUNT.username) : 'Sign in'}</button>
  </div>`;
}
function navHTML() {
...
function homeHTML() {
  ...
  let h = ACCOUNT && ACCOUNT.firstName ? `<div class="welcome"><span>Welcome back,</span><strong>${esc(ACCOUNT.firstName)}</strong></div>` : '';
  h += `<div class="library-tools"><label class="search"><span class="sr">Search lessons</span><input id="lesson-search" type="search" placeholder="Search lessons and concepts" value="${esc(S.search)}"></label><button class="btn ghost small-btn" data-act="clearSearch">${S.search ? 'Clear' : 'Browse all'}</button></div>`;
  if (S.search) {
    const q = S.search.toLowerCase(), matches = flat().filter((x) => { const l = LES[x.id]; return l.title.toLowerCase().includes(q) || l.cards.some((c) => JSON.stringify(c).toLowerCase().includes(q)); });
    h += `<section class="search-results"><h2 class="section-title">Search results</h2>${matches.length ? matches.map((x) => `<div class="result"><button class="result-main" data-act="node" data-id="${esc(x.id)}"><b>${esc(LES[x.id].title)}</b><span>${esc(x.unit.title)}</span></button><button class="bookmark ${P.bookmarks.includes(x.id) ? 'saved' : ''}" data-act="bookmark" data-id="${esc(x.id)}" aria-label="${P.bookmarks.includes(x.id) ? 'Remove bookmark' : 'Bookmark lesson'}">${P.bookmarks.includes(x.id) ? '★' : '☆'}</button></div>`).join('') : '<p class="fine">No lessons match that search.</p>'}</section>`;
  }
  C.tracks.forEach((t) => {
...
function practiceHTML() {
  const n = P.missed.length, due = dueReview().length, total = flat().length, doneN = Object.keys(P.done).length;
  const head = n
  ...
  return `<h1 class="ph">Practice</h1><p class="sub">A few minutes on what you've missed beats a long re-read.</p>${due ? `<div class="notice">${due} ${due === 1 ? 'lesson is' : 'lessons are'} ready for spaced review.</div>` : ''}${head}
...
function dashboardHTML() {
  const total = flat().length, doneN = Object.keys(P.done).length, saved = P.bookmarks.length, due = dueReview().length;
  const accuracy = Object.values(P.done).length ? Math.round(Object.values(P.done).reduce((sum, x) => sum + x.best, 0) / doneN) : 0;
  const badges = [doneN >= 1 ? 'First lesson' : '', doneN >= 5 ? 'Five lessons' : '', P.streak >= 3 ? 'Three-day streak' : '', P.xp >= 100 ? '100 XP' : ''].filter(Boolean);
  return `<h1 class="ph">Your progress</h1><p class="sub">A clear view of what you know and what to review next.</p><div class="tiles dashboard-tiles"><div class="tile" style="--c:var(--mint)"><b>Lessons</b><span>${doneN}/${total}</span></div><div class="tile" style="--c:var(--sun)"><b>Accuracy</b><span>${accuracy}%</span></div><div class="tile" style="--c:var(--coral)"><b>XP</b><span>${P.xp}</span></div></div><section class="card dashboard-card"><h2>${due ? 'Review is ready' : 'You are on track'}</h2><p>${due ? `${due} spaced-review ${due === 1 ? 'lesson is' : 'lessons are'} due.` : 'Complete another lesson to build your review queue.'}</p><button class="btn brand" data-act="goPractice">${due ? 'Review now' : 'Open practice'}</button></section><section class="card dashboard-card"><h2>Saved lessons</h2><p>${saved ? `${saved} lesson${saved === 1 ? '' : 's'} bookmarked for later.` : 'Bookmark lessons from Learn to find them here.'}</p>${P.bookmarks.map((id) => `<button class="saved-row" data-act="node" data-id="${esc(id)}">${esc(LES[id] && LES[id].title || id)}</button>`).join('')}</section><section class="card dashboard-card"><h2>Achievements</h2><p>${badges.length ? badges.join(' · ') : 'Your first badge is waiting.'}</p><button class="btn sun" data-act="shareProgress">Share progress</button></section>`;
}
function plansHTML() {
...
function landingHTML() {
  return `<main class="landing"><div class="landing-mark">${mascot(72, 'cheer')}<span>${esc(CFG.name.toLowerCase())}</span></div><p class="eyebrow">AI in bite-size lessons</p><h1>Build better AI instincts, one small lesson at a time.</h1><p class="landing-copy">Learn how AI works, write stronger prompts, and make safer decisions with short, practical lessons that fit into your day.</p><div class="landing-points"><div><b>15</b><span>guided lessons</span></div><div><b>3 min</b><span>per lesson</span></div><div><b>100%</b><span>self-paced</span></div></div><div class="stack"><button class="btn brand" data-act="startSignup">Create username</button><button class="btn ghost" data-act="startSignin">Sign in with username</button><button class="btn ghost about-link" data-act="aboutPublic">About Nibble</button></div><p class="landing-note">Your progress is saved to your username on this browser. No password required.</p></main>`;
}
function modalHTML() {
  const m = S.modal;
  if (m.kind === 'auth') {
    const signup = m.mode === 'signup';
    return `<div class="scrim"><div class="sheet" role="dialog" aria-modal="true" aria-labelledby="mt"><h2 id="mt">${signup ? 'Create your username' : 'Welcome back'}</h2><p>${signup ? 'Choose a username to save your Nibble progress in this browser.' : 'Sign in with your username to continue your saved lessons.'}</p>
      <form id="auth-form" class="account-form"><label for="auth-username">Username</label><input id="auth-username" name="username" type="text" autocomplete="username" pattern="[A-Za-z0-9._\\-]+" maxlength="24" required value="${esc(m.username || '')}"><button class="btn brand" type="submit">${signup ? 'Create username' : 'Sign in'}</button></form>
      ${m.error ? `<p class="auth-error" role="alert">${esc(m.error)}</p>` : ''}<button class="btn ghost" data-act="authToggle">${signup ? 'Already have a username? Sign in' : 'New here? Create a username'}</button><button class="btn ghost" data-act="close">Cancel</button></div></div>`;
  }
...
function bodyHTML(c) {
  if (L.fin) {
    return `<div class="done">${L.conf}${mascot(128, 'cheer')}<h1>${L.practice ? 'Practice complete' : 'Lesson complete'}</h1>
      ...
      <div class="tiles"><div class="tile" style="--c:var(--sun)"><b>XP earned</b><span>+${L.xp}</span></div><div class="tile" style="--c:var(--mint)"><b>Accuracy</b><span>${L.acc}%</span></div><div class="tile" style="--c:var(--coral)"><b>Day streak</b><span>${streakNow()}</span></div></div>${!L.practice ? `<div class="feedback"><p>Was this lesson useful?</p><button class="feedback-btn" data-act="feedback" data-v="yes">Yes</button><button class="feedback-btn" data-act="feedback" data-v="no">Needs work</button></div>` : ''}</div>`;
  }
...
function lessonHTML() {
  ...
  const hearts = L.practice || (P.settings && P.settings.relaxed) ? '' : `<div class="hp" aria-label="${P.pro ? 'Unlimited' : P.hearts} hearts">${ic.heart()}${P.pro ? '∞' : P.hearts}</div>`;
...
function render() {
  ...
  if (ae && ae.id === 'lesson-search') fk = '#lesson-search';
  else if (ae && ae !== document.body && ae.dataset && (ae.dataset.act || ae.dataset.tab)) {
...
const A = {
  ...
  aboutSignup() { S.publicAbout = false; S.modal = { kind: 'auth', mode: 'signup', username: '', error: '' }; render(); },
  startSignup() { S.modal = { kind: 'auth', mode: 'signup', username: '', error: '' }; render(); },
  startSignin() { S.modal = { kind: 'auth', mode: 'signin', username: '', error: '' }; render(); },
  account() { if (!ACCOUNT) { S.modal = { kind: 'auth', mode: 'signin', username: '', error: '' }; render(); return; } S.tab = 'account'; S.rs = true; render(); },
  authToggle() { S.modal.mode = S.modal.mode === 'signup' ? 'signin' : 'signup'; S.modal.error = ''; render(); },
  authSubmit(form) {
    const username = usernameKey(form.username.value);
    S.modal.username = username;
    if (username.length < 3) { S.modal.error = 'Use at least 3 letters, numbers, dots, dashes, or underscores.'; render(); return; }
    const all = store.get(ACCOUNTS_KEY) || {}, existing = all[username];
    if (S.modal.mode === 'signup') {
      if (existing) { S.modal.error = 'That username is already taken. Sign in instead.'; render(); return; }
      ACCOUNT = { username, createdAt: new Date().toISOString(), progress: P };
      all[username] = ACCOUNT; store.set(ACCOUNTS_KEY, all); store.set(ACCOUNT_KEY, username); save();
      S.modal = null; render(); toast('Username created. Your progress is saved here.'); return;
    }
    if (!existing) { S.modal.error = 'No local account exists for that username.'; render(); return; }
    ACCOUNT = { ...existing, username }; P = accountProgress(existing.progress); store.set(ACCOUNT_KEY, username); save();
  ...
  profileSubmit(form) {
    ...
    if (!firstName || !lastName) { toast('Add both your first and last name.'); return; }
    ACCOUNT = { ...ACCOUNT, firstName, lastName };
    save(); render(); toast('Profile saved.');
  },
  settingsSubmit(form) {
    P.settings = { largeText: form.largeText.checked, reducedMotion: form.reducedMotion.checked, relaxed: form.relaxed.checked };
    save(); render(); toast('Preferences saved.');
  },
  async reminderSubmit(form) {
  ...
  feedback(b) { if (L && L.id) { P.feedback[L.id] = b.dataset.v; save(); toast('Thanks for the feedback.'); } },
  node(b) {
  ...
  check() {
    ...
    if (!ok && !c.retry && !L.practice) { addMissed(L.id, c.id); scheduleReview(L.id, 1); }
    if (ok && !c.retry && L.practice && c._m) removeMissed(c._m);
    save(); render();
  },
  next() {
    if (L.phase === 'fb' && !L.ok && !P.pro && !(P.settings && P.settings.relaxed) && P.hearts === 0 && !L.practice) { L.out = true; render(); return; }
    advance();
  },
...
document.addEventListener('input', (e) => {
  if (e.target.id === 'lesson-search') { S.search = e.target.value; render(); return; }
...
(async function boot() {
  try {
    ...
    Object.keys(accounts).forEach((key) => {
      ...
      if (!record.username) {
        const username = usernameKey(record.email ? record.email.split('@')[0] : key) || 'learner';
        if (!accounts[username]) accounts[username] = { ...record, username };
        delete accounts[key].email;
        delete accounts[key].passwordHash;
        if (username !== key) delete accounts[key];
      }
    });
    store.set(ACCOUNTS_KEY, accounts);
    const migratedActive = active && accounts[active] ? active : active && usernameKey(active.split('@')[0]);
    ACCOUNT = migratedActive && accounts[migratedActive] ? accounts[migratedActive] : null;
    if (ACCOUNT) store.set(ACCOUNT_KEY, ACCOUNT.username);
    P = ACCOUNT ? accountProgress(ACCOUNT.progress) : accountProgress(store.get(KEY));
    setInterval(() => {
      if (!ACCOUNT || !P.reminder || !('Notification' in window) || Notification.permission !== 'granted') return;
      const t = new Date(), at = String(t.getHours()).padStart(2, '0') + ':' + String(t.getMinutes()).padStart(2, '0');
      const stamp = dayKey(t) + ' ' + at;
      if (at === P.reminder && P.lastReminder !== stamp) { P.lastReminder = stamp; save(); new Notification('Time for a Nibble', { body: 'Keep your AI learning streak going.' }); }
    }, 30000);
